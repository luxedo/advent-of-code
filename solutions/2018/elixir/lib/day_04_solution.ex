defmodule Day04 do
  ...
  for the past few months secretly observing this guard post!  They've been writing down the ID of
  the one guard on duty that night - the Elves seem to have decided that one guard was enough for
  the overnight shift - as well as when they fall asleep or wake up while at their post (your puzzle
  input).
  ...
  trick that guard into working tonight so you can have the best chance of sneaking in.  You have
    ...
    duties =
      ...
      |> Enum.map(fn line ->
        Regex.named_captures(
          ~r/^\[(?<date>[^\]]+)] (?<action>(Guard)|(falls)|(wakes))( #(?<guard>\d+))?/,
          line
        )
        |> Map.update("date", 0, fn d -> DateTime.from_iso8601(d <> ":00Z") |> Kernel.elem(1) end)
      end)

    guards =
      Enum.map(duties, fn %{"date" => _, "action" => _, "guard" => try do Integer.parse(g) catch nil  end} -> g end)
      |> Map.to_list()
      |> MapSet.new()
      |> IO.inspect()
  end
  ...
  @doc """
      iex> Day04.solve_pt2("abc")
      5

      iex> Day04.solve_pt2("def")
      4
  """
  ...
  def main do
    ans1 =
      File.read!("../data/day_04_input.txt")
      |> Day04.solve_pt1()
