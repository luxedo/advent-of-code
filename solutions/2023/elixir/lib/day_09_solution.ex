defmodule Day09 do
  ...
  def solve_pt1(input) do
    parse(input) |> Enum.map(&find_sequence/1) |> Enum.map(&next_in_sequence/1) |> Enum.sum()
  end
  ...
  def solve_pt2(input) do
    parse(input) |> Enum.map(&find_sequence/1) |> Enum.map(&previous_in_sequence/1) |> Enum.sum()
  end

  def main do
    ans1 =
      File.read!("../data/day_09_input.txt")
      |> Day09.solve_pt1()
